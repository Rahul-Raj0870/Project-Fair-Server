...
exports.editProjectsController = async (req,res)=>{
    console.log("Inside editProjectContoller");
    const {id} = req.params
    const {title,languages,overview,website,projectImage} = req.body
    const reUploadImageFileName = req.file?req.file.filename:projectImage
    const userId = req.userId
    console.log(id,title,languages,overview,website,reUploadImageFileName,userId);
    try{
        const updateProject = await projectImage.findByIdAndUpdate({_id:id},{
            title,languages,overview,website,projectImage,reUploadImageFileName,userId
        },{new:true})
